...
public class AIController implements Controller, Constants {
  ...
  static CommandNextAngle commandNextAngle = new CommandNextAngle();
  static private int turn = 3; //nextflagを取った場合の例外処理数
  static private int turnAngle = 0;
  ...
  public static int NormalDrive(SensorModel inputs){
	...
	double angleToNext = inputs.getAngleToNextWaypoint();
	...
	// if(turnAngle > 7 && nextwaydistance > 0.3){
	//   command = commandNextAngle.getCommand(command, inputs, turnAngle);
	// }
  ...
  static public int getCommandNextDistance(int basedCommand,SensorModel inputs, boolean except){
	...
	if(except){
	  /*  if(0.45 < nextwaydistance && nextwaydistance < 0.7){
		...
		speed = 4;
		dev = 1;
		// if(-Math.PI / 6 < angle && angle < Math.PI / 6){
		//   speed = 5;
		//   dev = 1;
		// }
